...
- Lifecycle completion uses fresh MCSManager status for `stopped` and Minecraft Server List Ping with the latest instance address for readiness; `start` may begin without an address, and `restart` must observe an interruption before accepting Ping recovery.
